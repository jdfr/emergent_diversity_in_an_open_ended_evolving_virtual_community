function [raster, offset, counts, dimension, isLeaf, maxiy, leafs]=treeRaster(axiom, mode)
...
if isstruct(mode)
  ...
  samelevelrule = not(isfield(mode, 'sameLevelRule')) || mode.sameLevelRule;
  mode        = mode.shadowing;
else
  notjustPaint = true;
  samelevelrule = true;
...
replacesamelevel = notjustPaint && samelevelrule;
switch mode
    case 'canonical'
    case 'probabilistic'
...
% if(useTransparency)
    ...
    rasterLeaveValSameLevel = int8(3); % Leaves of the same level can be deleted by G's of that level or immediately upper
% else
    ...
    switch cmdT
        ...
        case 'G'
            ...
            if samelevelrule
              ...
              if (tip~=rasterLeaveVal)
                raster(y2,x2) = rasterLeaveValSameLevel;
              end
            else
              %raster(indexes(raster(indexes)~=rasterLeaveVal)) = rasterFillVal;
              raster(indexes) = rasterFillVal;
              raster(y2,x2) = rasterLeaveVal;
            end
        ...
        case ']' % return to former position (last save)
          if replacesamelevel 
            if npx<1e6
              raster(raster==rasterLeaveValSameLevel) = rasterLeaveVal;
            else
              raster(find(raster==rasterLeaveValSameLevel)) = rasterLeaveVal; %#ok<FNDSB>
            end
          end
            stkPtr = stkPtr -1 ;
            xT = stack(stkPtr,1);

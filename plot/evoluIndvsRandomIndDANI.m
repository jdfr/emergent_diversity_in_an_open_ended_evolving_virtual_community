function evoluIndvsRandomInd(basedir, symbols)
...
plotXTickLabels = 'A|E|I|DA|DN|DAE|DNE|DSE';
...
genome = poph.genome(poph.generation == numGenerations);

sgenome = size(genome,1);
%sgenome = 2;
...
if(meanAnid > 1)
    genomeRandom = generateGenomeRTipo2(sgenome,round(meanGs),round(meanLongGenome),round(meanBra), round(meanAnid));
else
    genomeRandom = generateGenomeRTipo3(sgenome,round(meanGs),round(meanLongGenome),round(meanBra));
end
...
for i = 1 : sgenome
    numGsR(i) = sum(genomeRandom{i} == 'G');
    numBraR(i) = sum(genomeRandom{i} == '[');
    numSillyBraR(i) = sum(genome{i}(1:end-1) == '[' & genome{i}(2:end) == ']'); 
    numAnidR(i) = max(cumsum((genomeRandom{i} == '[') + (-1 * (genomeRandom{i} == ']'))));
    longGenomeR(i) = length(genomeRandom{i});
end

meanGsR = mean(numGsR);
meanBraR = mean(numBraR);
meanSillyBraR = mean(numSillyBra);
meanAnidR = mean(numAnidR);
meanLongGenomeR = mean(longGenomeR);

times = round(10000/sgenome);
%times = 20;
mode = 'canonical';

fLog = fopen(sprintf('%sRobustez_%s.log',basedir, datestr(now, 30)), 'w');
fprintf(fLog, 'Evolved genomes: meanGs=%f meanBra=%f meanSillyBra=%f meanAnid=%f meanLongGenome=%f\r\n', meanGs, meanBra, meanSillyBra, meanAnid, meanLongGenome);
fprintf(fLog, 'Random Genomes: meanGs=%f meanBra=%f meanSillyBra=%f meanAnid=%f meanLongGenome=%f\r\n\r\n', meanGsR, meanBraR, meanSillyBraR, meanAnidR, meanLongGenomeR);
...
disrupR = zeros(numOpFuncs, sgenome, times);
% random genome
for i = 1 : sgenome

function [especies comEspeciesAnte comEspeciesSig]= relacionarSpeciesNEWCOMPLEXITY(arg1,numGenerations, structcomplex)
...
for i = 1 : numGenerations-1
    fprintf('VAMOS POR LA GENERACION %d\n', i);
    ...
    comLZfigureLongitudA = structcomplex.complexities{structcomplex.gs==i};
...
%     comLZfigureLongitudA = eval(sprintf('comLZfigureLongitud%03d', i));
    ...
    if (i-1)==0
      init = 'G';
      level = 9;
       [raster, offset, counts, dimension]=treeRaster(init, struct('shadowing', 'canonical', 'angle', 22));
       comLZfigureLongitudS = getCompression(raster, dimension, level);
    else
      comLZfigureLongitudS = structcomplex.complexities{structcomplex.gs==(i+1)};
    end

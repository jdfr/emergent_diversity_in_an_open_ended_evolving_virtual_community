function processGeneration242NEWCOMPLEXITY(basedir, P, minGenome, proto, numGroups, lgenome, lmingenome)
...
comLZfigureLongitud = zeros(size(proto));
level = 9;
for k=1:numel(comLZfigureLongitud)
  comLZfigureLongitud(k) = getCompression(proto{k}, [max(proto{k}{1}), max(proto{k}{2})], level);
end
...
if paint
  ...
  clear img1 img2;
  f = 'lsystemdani\figurasrene\pruebanew.png';
  imwrite(img, f, 'png');
end
